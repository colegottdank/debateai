 ...
 * Only returns debates that have been scored (completed) — these are the
 * interesting ones with full transcripts and results.
 *
 * Query params:
 *   sort: recent | top_scored | most_messages (default: recent)
 *   limit: 1-50 (default: 20)
 *   offset: pagination offset (default: 0)
 *   category: optional topic keyword filter
 */
export const GET = withErrorHandler(async (request: Request) => {
  ...
  // Only show debates that have been scored (score_data IS NOT NULL and has debateScore)
  ...
  const debates = result.result.map((row) => {
    ...
    // Parse messages to get count + preview
    let messageCount = 0;
    let previewMessage = '';
    try {
    ...
    try {
      ...
      if (sd?.opponentStyle) {
        opponentStyle = sd.opponentStyle;
      }
    } catch {
      // Ignore
    }
    ...
    return {
      id: r.id,
      topic: r.topic,
      opponent: opponentStyle || r.opponent,
      messageCount,
      previewMessage,
